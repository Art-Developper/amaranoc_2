...
export default function About() {
  ...
  useEffect(() => {
    // 👇 Ավելացված է api/ fetchData-ի մեջ
    fetchData("api/about").then((data) => {
      if (data) {
        setSectionData(data);
      }
    });
  ...
  return (
    <>
      <div className="relative h-screen w-full">
        ...
        <header className="relative z-[60] flex justify-between items-center px-10 py-7 max-w-[1600px] mx-auto">
          ...
          <div className="hidden min-[1321px]:flex items-center justify-center gap-10">
            ...
            <Link className={linkClass("/sales")} href="/sales">
              Զեղչեր
            </Link>
            <Link className={linkClass("/service")} href="/service">
              Ծառայություններ
            </Link>
            <Link className={linkClass("/about_us")} href="/about_us">
              Մեր մասին
            </Link>
          </div>
          ...
          <div className="flex gap-5 justify-end items-center text-white">
            ...
            <div className="hidden min-[1321px]:flex items-center gap-3">
              {loading ? null : user ? (
                <div className="flex items-center gap-3">
                  <Link
                    href="/userPage"
                    className="text-[14px] font-bold text-white border-b border-orange-500"
                  >
                    {user.name}
                  </Link>
                  <button
            ...
            <button
              className="min-[1321px]:hidden p-2 hover:text-orange-400 transition"
      ...
      />
      <div
        className={`fixed top-0 right-0 h-full w-[350px] sm:w-[450px] bg-white z-[110] shadow-2xl p-10 flex flex-col transform transition-transform duration-500 ${isMenuOpen ? "translate-x-0" : "translate-x-full"}`}
      >
        <button
          className="absolute top-8 right-8 w-11 h-11 border border-gray-100 rounded-full flex items-center justify-center text-gray-400"
          onClick={() => setIsMenuOpen(false)}
        >
          <X size={24} />
        </button>
          ...
          {user ? (
            <div className="flex flex-col gap-6">
              ...
              <button
                ...
                className="text-[20px] font-bold text-red-500 text-left"
              >
                Դուրս գալ
              </button>
            </div>
            ...
            <React.Fragment key={section.id}>
              ...
              {(index === 1 || index === 4) && (
                <div className="w-full pt-10">
                  <img
                    src={
                      index === 1 ? "/image/second.webp" : "/image/third.webp"
                    }
                    alt="Banner"
        ...
        <div className="relative z-10 max-w-6xl mx-auto border border-gray-700 rounded-3xl p-10 text-center backdrop-blur-sm">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <input
              ...
              className="bg-white/10 border border-gray-500 rounded-2xl px-6 py-3 w-full md:w-64 outline-none focus:border-orange-400 text-white"
            />
            <input
              type="tel"
            ...
            <input
              ...
              className="bg-white/10 border border-gray-500 rounded-2xl px-6 py-3 w-full md:w-64 outline-none focus:border-orange-400 text-white"
            />
            <button className="bg-orange-400 text-black px-10 py-3 rounded-2xl font-bold hover:bg-orange-500 transition-all">
              Ուղարկել
            </button>
          </div>
        </div>
      ...
      <footer className="bg-[#101623] text-white pt-10">
        <h2 className="text-center text-3xl mb-10 tracking-widest font-light uppercase">
          Կոնտակտներ
        </h2>
        <div className="flex flex-wrap justify-center gap-10 px-4 mb-10">
          ...
          <div className="flex items-center gap-2 uppercase tracking-wide">
            ...
            <span className="text-sm">amaranoc.info@gmail.com</span>
          </div>
          <div className="flex items-center gap-2 uppercase tracking-wide">
        ...
        <div className="text-center text-gray-500 text-xs pb-10 space-y-2">
          <p className="underline cursor-pointer hover:text-orange-500 transition">
